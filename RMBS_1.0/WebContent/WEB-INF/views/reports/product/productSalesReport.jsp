...
<c:url value="/productSalesReports/download.html" var="downloadUrl" />
<c:url value="/download/token.html" var="downloadTokenUrl" />
<c:url value="/download/progress.html" var="downloadProgressUrl" />
<c:url value="/openProductSalesView.html" var="purchaseViewUrl"/>
<!DOCTYPE html>
<html>
<head>
		...
		/*function requestSubmitionByAjax() {
			...
			$.post('purchaseFile.html', formData, function(result) {
				...
				/* $('#msgbox').text('Processing download...');
         			$('#msgbox').dialog( 
						...
						window.location = '${downloadUrl}' + '?token=' + token
								+ '&type=' + type +'&proName='+$('#proName').val()+ '&bId=' + bid + '&cat='
				...
				$("#catId").blur(function(){
					...
					var data = {bId:$('#bId').val(),catId:$('#catId').val()};
				
					$.post('getProdctNamesFromInv.html',data,function(result){
						//alert(productInventoryModel);
						//alert('name '+productInventoryModel.id);
						//$('#proName').val(productInventoryModel.productModel.id);
						if(!jQuery.isEmptyObject(result)){
								$('#proName option').remove();
								var o = new Option("--select--,0");
								$(o).html('--Select--');
								$('#proName').append(o);
								$.each(result,function(obj){
						 			var o = new Option();
									$(o).html(result[obj].productModel.name);
									//alert('hhyy7 '+result[obj].id);
									$("#proName").append(o);
								});
						}else{
							$('#proName option').remove();
...
<body>
	<div class="container">
		<div>
			<h3>Product Sales Reports Form</h3>
		</div>
		<form:form class="wrapper" id="formData">
			...
			<div>
				<label><form:label path="category">Category</form:label></label>
					<form:select path="category" id="catId">
					<form:option value="${catId}">${category}</form:option>
					<form:option value="7">MEDICAL</form:option>
					<form:option value="8">GENERAL</form:option>
				</form:select>
			</div>
			<div>
				<label><form:label path="productName">Product Name</form:label></label>
				<form:select path="productName" id="proName">
					<form:option value="${proNameId}">${product}</form:option>
					<!--<form:options items="${proNames}"/>-->
				</form:select>
			</div>
			<div>
