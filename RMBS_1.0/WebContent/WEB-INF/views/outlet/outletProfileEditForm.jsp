...
<h3>Outlet Profile edit form</h3>
<div class="wrapper">
<form:form method="POST" action="updateOutlet.html;"  commandName="command" id="registerform1" class='outletregistrastion'>
	...
	<div><span class="caption">Branch:</span><c:out value="${sessionScope.user.branchBean.name}"/></div>
	<div><span class="caption">Organization:</span><c:out value="${sessionScope.user.branchBean.organizationBean.name}"/></div>
	<div><span class="caption">Email: </span><c:out value="${sessionScope.user.branchBean.resourceBean.addressBean.email}"/></div>
	<div><span class="caption">Contact No: </span><c:out value="${sessionScope.user.branchBean.resourceBean.addressBean.mobile}"/></div>
	<div><span class="caption">Address: </span><c:out value="${sessionScope.user.branchBean.resourceBean.addressBean.address}"/></div>
   ...
   <div>
	...
	<div>
		...
		<form:input path="firstName" class="names" id="fname" value="${outlet.firstName}" />
	</div>
	
    <div>
		<label><form:label path="lastName" >Owner Last Name</form:label></label>
		<form:input path="lastName" class="names" id="lname" value="${outlet.lastName}" />
	</div>
	<div>
		<label><form:label path="userName" >User Name</form:label></label>
		<form:input path="userName" id="uname" value="${outlet.userName}" readOnly="true"  onblur="return userid_validation(this,5,12);" />
	</div>
	<div  style="display: ;">
		<label><form:label path="gender" >Gender</form:label></label>
		<form:input path="gender" id="sex" value="${outlet.gender}" readonly="true"/>
	</div>
	
	<div>
		<label><form:label path="dob" >Date of Birth</form:label></label>
		<form:input path="dob" id="dob" placeholder="Select" value="${outlet.dob}" maxlength="0"/>
	</div>
	<div>
		<label><form:label path="resourceBean.addressBean.mobile" >Mobile</form:label></label>
	    <form:input path="resourceBean.addressBean.mobile" class="phno" id="phno1" value="${outlet.resourceBean.addressBean.mobile}" maxlength="15"/>
	</div>
		<div>
		<label> <form:label path="resourceBean.addressBean.email" >Email Id</form:label></label>
		<form:input path="resourceBean.addressBean.email" id="pmail" class="email" value="${outlet.resourceBean.addressBean.email}" placeholder="abcd@gmial.com"  />
	</div>
	</div>
	</div>
	<div>
