...
<script type="text/javascript" charset="utf-8">
        ...
        //	   alert('calling'+val+' '+val1);
        	   var data = {id:val,flag:val1,task:"sub"};
        	   $.get('getPurchaseLineItems.html',data, function(result){
        	//     alert(result);
        		   $('.col2').html(result);
        	   });
           }//+'&task=page'
		...
		<table id="example" class="display"
			...
			<tbody>
				...
				<c:set value="${sessionScope.user.id}" var="id" />
				     ...
				     <c:forEach items="${purchaseOrders}" var="purchaseOrder" varStatus="counter">
					...
					<tr id="<c:out value="${purchaseOrder.id}"/>1">
						...
						<td align="center"><a href="#" title="View Purchase Line Items"
							onclick="purchaseOrderIdByPurchaseLineItems(<c:out value="${purchaseOrder.id}"/>,0);"><img
								src="images/open1.jpg" alt="open" height="18px" width="18px" /></a>&nbsp;<a
							href="#" title="View Purchase Order Invoice Print Copy"
							onclick="getInvoice(<c:out value="${purchaseOrder.id}"/>);"><img
								src="images/print2.png" alt="print" height="18px" width="18px" /></a>
							<c:if test="${sessionScope.user.resourceBean.roleBean.id==branch}">	
								<a href="#" title="View purchase return" onclick="purchaseOrderIdByPurchaseLineItems(<c:out value="${purchaseOrder.id}"/>,1);"><img
								src="images/SRO.png" alt="open" height="18px" width="18px" /></a>
								</c:if>
								</td>
...
<script type="text/JavaScript">
    $(function(){
        $('.page_nav a').click(function(e) {
         alert($(this).text());
         location.replace('<%=request.getContextPath()%>/purchaseOrderList.html?pg='+$(this).text()+'&flag=task_main');  
